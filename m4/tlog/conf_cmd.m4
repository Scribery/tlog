...
m4_define(
    ...
    `
        m4_printl(
           `            if (optarg == NULL) {',
           `                fprintf(stderr, "Empty %s option value.\n", optname);',
           `                tlog_'M4_PROG_NAME()`_conf_cmd_help(stderr, progname);',
           `                grc = TLOG_RC_FAILURE;',
           `                goto cleanup;',
           `            }',
           `            else if (sscanf(optarg, "%" SCNd64 " %n", &val_int, &end) < 1 ||',
...
m4_define(
    ...
    `
        ...
        m4_printl(
           ...
           `                size_t i;',
           `                for (i = 0;',
           `                     i < TLOG_ARRAY_SIZE(list) && optarg != NULL && strcmp(optarg, list[i]) != 0;',
           `                     i++);',
           `                if (i >= TLOG_ARRAY_SIZE(list) || optarg == NULL) {',
           `                    fprintf(stderr, "Invalid %s option value: %s\n",',
           `                            optname, optarg == NULL ? "<empty>" : optarg);',
           `                    grc = TLOG_RC_FAILURE;',
           `                    tlog_'M4_PROG_NAME()`_conf_cmd_help(stderr, progname);',
           `                    goto cleanup;',
           `                }',
